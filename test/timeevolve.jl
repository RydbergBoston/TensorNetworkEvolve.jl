...
@testset "smatrix apply" begin
    ...
    v2 = TensorAD.DiffTensor(variables(p1))
    _complex(x::AbstractVector) = [Complex(x[2i-1], x[2i]) for i=1:length(x)÷2]
    h2 = ForwardDiff.hessian(x->TensorNetworkEvolve.normalized_overlap(p1, _complex(x)[1:length(v1.data)], _complex(x)[length(v1.data)+1:end])[], reinterpret(Float64, vcat(v1.data, v2.data)))
